...
public class ObtainStepData implements SensorEventListener {
    ...
    float[] initPoint1 = {110.0f, 1250.0f};//{80.0f, 470.0f};//{200.0f, 500.0f};//{330.0f, 120.0f};//{550.0f, 1150.0f}{56,700};////走廊需要改变的初始点坐标
    float[] initPoint0 = {95.0f, 500.0f};//{56,700};////
    public static float[] curCoordsOfStep = {110.0f, 1250.0f};//{80.0f, 470.0f};//{200.0f, 500.0f};//{330.0f, 120.0f};//需要改变的初始点坐标
...
//        	/alpha = 0.25f 将后面的新计算的值赋给out[i]
    ...
    public void correctStep() {
        ...
        if (SettingActivity.sMapId == 0) {
            ...
            curCoordsOfStep[0] = 110.0f;//80.0f;//200.0f;//300.0f//56;//95;
            curCoordsOfStep[1] = 1250.0f;//470.0f;//500.0f;//120.0f//700;//1181;
        }
