...
public class ObtainOffLineLearnStepData implements SensorEventListener {
    ...
    public ObtainOffLineLearnStepData(Context context, TextView stepCount, TextView stepLength, TextView mMMapStepLength, TextView mDegree, float[] coordPoint1, float[] coordPoint2) {
        ...
        // coordPoint1[0] = 160.0f; coordPoint1[1] = 150.0f//起点
        //coordPoint2[0] = 160.0f; coordPoint2[1] = 1250.0f;//终点）
        mDistanceOnMap = (float) Math.sqrt(Math.pow(coordPoint1[0] - coordPoint2[0], 2) + Math.pow(coordPoint1[1] - coordPoint2[1], 2));//40.0f;//(float) Math.sqrt(Math.pow(coordPoint1[0] - coordPoint2[0], 2) + Math.pow(coordPoint1[1] - coordPoint2[1], 2));
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            if (mCurTimeAcc - mLastTimeAcc > 50) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_MAGNETIC_FIELD) {
            ...
            //40改为80改为100try200
            if (mCurTimeMag - mLastTimeMag > 50) {
    ...
    public void stopStep() {
        ...
        mScale = 0.2682f;//averageMapLength / averageLength; 1090/4064.23; 1090=1250-160;4064.23是手机读数
        //显示平均步长
